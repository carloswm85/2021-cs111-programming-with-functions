...
def main():
    ...
    # Counts and prints the number of marriages
    # that each person had in his or her lifetime.
...
def print_death_age(people_dict):
    ...
    for person in people_dict.values():
        ...
        birth = person[BIRTH_YEAR_INDEX]
        death = person[DEATH_YEAR_INDEX]
        age = death - birth
        print(f'Name: {name}; Birth: {birth}; Death: {death}; Age of Death: {age}')
...
def print_marriages(marriages_dict, people_dict):
    ...
    for key, marriage in marriages_dict.items():
        ...
        for key, person in people_dict.items():
            if key == husband_key:
                husband_name = person[NAME_INDEX]
                husband_age = wedding_year - person[BIRTH_YEAR_INDEX]
            if key == wife_key:
                wife_name = person[NAME_INDEX]
                wife_age = wedding_year - person[BIRTH_YEAR_INDEX]
        print(f'{husband_name} {husband_age} > {wedding_year} < {wife_name} {wife_age}')


def count_marriages(marriages_dict, people_dict):
    """Add to your program a function named count_marriages
    that counts and prints the number of marriages that
    each person had in his or her lifetime. According to
    the data, who married the most times?
    """
    print('Number of times each person was married:')
    for key, person in people_dict.items():
        name = person[NAME_INDEX]
        person_key = key
        marriages = 0
        for key, marriage in marriages_dict.items():
            wife_key = marriage[WIFE_KEY_INDEX]
            husband_key = marriage[HUSBAND_KEY_INDEX]
            if person_key == wife_key or person_key == husband_key:
